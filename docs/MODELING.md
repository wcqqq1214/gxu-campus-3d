...
重建已有 OSM 定位的 31 片室外单场，其中西校园集中区 16 片。其余为原有周边和零散球场，不应把 31 片理解为全校总数。学校 [2024 年体育设施介绍](https://tyxy.gxu.edu.cn/info/1166/2372.htm)记载东区 15 片、西区 16 片；当前地图没有东校园田径场旁的独立篮球场轮廓；后续依据校方导览图补绘西侧 15 片估算场地，布局及坐标不宣称实测，详见 [东校园篮球场](EAST_BASKETBALL.md)。两块仅标注整体范围的活动区继续保留铺地，去掉误导性的超大单场外框和中线；室内篮球馆保持原建筑。
...
`public/data/basketball.json` 保存定位、原轮廓、估算轮廓、分组、地形补片及来源。基础 GLB 按五个场地组聚合，避免每个篮架成为单独绘制对象；源文件保留 46 个独立球场（31 个 OSM 定位、15 个资料约束估算）及具名的标线、篮架、篮网分组。球场由“运动场地”图层控制，不加入搜索或自动巡游。
