...
本页对应 [实施计划](CAMPUS_REFINEMENT_PLAN.md) 的 S2。**2026-09-21最新状态：加入[新结构平台首批体量](CIVIL_PLATFORM_MASSING.md)后共22个校准对象，土木学院1栋通过[首轮整栋校准](CIVIL_WHOLE_REVIEW.md)，其余21个仍为部分校准。** 大厅与九层北楼属于同一复合对象，不重复计数。首轮通过允许明确登记的估算与未知项，不是实测或2026年现状认证。

以下保留各对象首次及局部校准记录，当时建立21条部分对象校准记录，其中原有 8 栋相对初始参数调整主体高度，动物科学技术学院增加高入口核心与低侧翼分段，农学院南侧凸部校准为低门廊并将入口内退到主体墙。首批 8 栋之后新增东苑、碧湖苑两处食堂，详见[食堂校准](DINING_BUILDINGS.md)。**这不是 21 栋完整实测，也尚未完成首批 20–30 栋校准。** 未定位的入口、背立面、附楼范围和层高仍单独列为待核对项。

土木建筑工程学院 `relation/12875606` 新增南侧东端玻璃门厅、外挑顶板和上部玻璃立面，撤去原东侧中段的默认入口。`flushEntrance` 可选 `canopy` 明确宽度、深度与厚度，顶板下缘跟随玻璃顶高；原不带顶板入口保持。主体八层/26.4米仍沿用原估算，西南低附楼、后翼、端部斜墙和完整接路未完成，见[土木学院记录](CIVIL_BUILDING.md)。

土木学院后续将[西南C形低附楼](CIVIL_ANNEX_MASSING.md)拆为三层、10.8米估算体量，修正与主楼等高的示意错误；原轮廓、内院和入口保持。底层口径、精确分界、局部高屋面及细分级仍待核对，不据此增加整栋完成数。
...
艺术学院新增主楼三层、临湖附楼四层及西侧通透雨棚的照片约束估算，见[本批参数与验证](ARTS_BUILDINGS.md)。其中雨棚与主楼属于同一建筑空间，不按两栋计入整栋完成数。 随后新增[临湖外楼梯](ARTS_LAKESIDE_STAIR.md)记录，将默认五层封闭圆筒改为四层附楼的开放折返梯及三层连接平台；楼梯与附楼同样不拆算为整栋完成数。

办公北楼新增中央四层、两侧三层的体量分段，最高层数依据校方项目册，分界和层高标为估算；北侧中央[玻璃门厅、上部窗带及前场连接](OFFICE_NORTH_ENTRY.md)已补充，侧翼和其他立面仍待核对；体量依据见[办公北楼体量](OFFICE_NORTH_MASSING.md)。
...
西楼楼梯标识墙随后补充[十二处小窗](MATH_CENTER_STAIR_WINDOWS.md)。显式 `panels` 可与 `windows: false` 并用，保留指定窗面并停止生成默认窗列；窗组数量有照片依据，尺寸与锚点为估算。
...
- 整栋 `roof` 支持 `type` 和 `rise`；默认矩形屋脊跟随映射轮廓的局部轴，未知字段直接报错。分部的有界显式屋脊见下项。多体量坡屋面在各部分内生成，禁止用整栋外接矩形封闭凹口和内院。
...
- `openCorridor` 可锚定原内院边界，退进仍须完全位于所属分部。可选 `frontProfile` 使用端墙之间的比例/内退深度对，表达向内折线的楼板与实栏；两端深度为零、点序前进、保留至少0.6米后墙净距，最多64点。限保留底层的完整上部楼层，不与显式立柱或镂空栏杆混用。原直线屋顶及底层墙顶封口保持，示例见[土木主楼北廊](CIVIL_NORTH_GALLERY.md)。
- `entrances` 通过 `polygon/ring/edge/t` 定位在原外边，记录宽度和主次。方向由外边法向解析，北为 0°、顺时针递增；普通封闭建筑必须恰有一个主入口，门宽不能超过所在外边。无资料时保留明确标为估算的最长边入口。唯一允许显式空列表的情况是 `building=roof` 且存在全部带 `openBelow` 的明确分段，用于独立映射的开放雨棚；不在雨棚边缘生成假门。非列表输入一律拒绝。
...
贴墙玻璃门厅采用`entrances.flushEntrance`：不生成默认外挑雨棚或台阶，明确玻璃高、门框、柱和门槛高，受所在实体分段高度约束。`front-connection`可通过`pathWidth`连接这类入口，宽度需容纳门口并避开中央柱；铺面标高从门槛接到实际道路。`panels.solid`用于上部实墙分区，不生成玻璃或花格。
