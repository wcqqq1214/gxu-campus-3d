...
[校方2020年门厅改造采购公告](https://www.gxu.edu.cn/info/1006/24949.htm)明确涉及门厅车道及其顶面，支持入口附近存在有顶车行空间的线索。公告未提供可定位的车道范围、上下层标高或净高，尚不能确认它与 `way/842784663` 的对应关系；因此仍不恢复已撤回的实心台阶，也不直接生成下穿通道。

原始OSM快照中，该服务道路版本1的编辑时间为2020-08-31，只有 `highway=service`，没有 `bridge`、`tunnel` 或 `layer` 标签。标签缺失不证明道路一定与台阶同层。后续应同时核对有顶车道与人行台阶，二维重叠继续作为需要复核的信号，而非真实冲突的最终判定。来源登记为 `gxuMaterialsEntrance2020`；本次只补证据，不变更该楼模型。
