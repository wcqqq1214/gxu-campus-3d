"""Ray-check the sourced recessed entrance under a multistorey core in source, base and shipping near chunk.

Optional --check-root=... checks an older model against the current intended
building data. It must fail on the previous solid entrance extrusion.
"""
...
PREFIX=next((a.split('=',1)[1] for a in sys.argv if a.startswith('--report-prefix=')),'s3-animal')
...
e=b['form']['entrances'][0];p=next(p for p in b['form']['parts'] if p['id']=='entrance-front')
...
def check(ts,tolerance,label,ground,windows):
    ...
    assert roof and abs(roof[0][0].z-z-p['height'])<=tolerance,(label,'upper core roof height changed',roof[0][0].z if roof else None)
    ...
    # Open space between the two side piers, ending before the rear door.
    for fraction in [.14,.5,.86]:
        point=front+u*((fraction-.5)*width)+n*.2;point.z=z+1.6
        hits=ray(ts,point,-n,depth-.1)
    ...
    facade=next(f for f in b['form']['facades'] if f.get('edge')==11 and f['part']=='entrance-front')
    length=math.dist(facade['start'],facade['end']);grid=facade['rule'].get('windowGrid')
    count=max(1,int(length/4));fraction=.5/count;level=1
    if grid:
        fraction=(grid['edgeInset']+(length-2*grid['edgeInset'])/grid['columns']/2)/length
        level=grid['firstLevel']
    window=Vector((*[facade['start'][i]+(facade['end'][i]-facade['start'][i])*fraction for i in (0,1)],z+(level+.56)*facade['height']/facade['levels']))
    assert ray(windows,window+n*.8,-n,1),(label,'exposed second-floor window above portico is missing')
    # The new split must not introduce a parapet across the shared roof.
    seam=Vector((*e['center'],z+p['height']+2))
    for offset in [-width*.2,0,width*.2]:
        # Sample inside both roof faces, avoiding exact float/Draco edges.
        # A spurious 0.25 m parapet still intersects both rays.
        for side in [-.04,.04]:
            hit=ray(ts,seam+u*offset+n*side,Vector((0,0,-1)),3)
            assert hit and abs(hit[0][0].z-z-p['height'])<=tolerance,(label,'spurious shared-roof parapet',offset,side)
    stair_heights=[];ground_clearances=[]
    for i in range(e['steps']):
        point=front+n*((i+.5)*.3);point.z=z+1.4
        hit=ray(ts,point,Vector((0,0,-1)),2)
        base=e.get('stepBaseHeight',0)
        expected=base+(e['platformHeight']-base)*(e['steps']-i)/e['steps']
    ...
    return {'canopyHeight':roof[0][0].z-z,'floorHeight':floor[0][0].z-z,'openBayRays':3,'columns':2,'rearDoor':True,'exposedWindowAbovePortico':True,'stairHeights':stair_heights,'stairClearanceAboveGroundOrPaving':ground_clearances,'passed':True}
