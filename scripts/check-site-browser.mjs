...
const cameras = JSON.parse(await fs.readFile(path.join(root, 'docs/model-checks/refinement/cameras.json')));
const camera = cameras.find((c) => c.id === 'library-side');
...
async function visit(name, near, light = 'day') {
  ...
  if (near) hash.set('place', 'library');
...
try {
  ...
  if (low.loadedDetails.includes('landmark-library')) throw new Error('Base check unexpectedly loaded library detail');
  await visit('near', true);
  await layers();
  await layer('地点名称', false);
  const near = await capture('near-labels-off');
  if (!near.loadedDetails.includes('landmark-library')) throw new Error('Near library missing');
