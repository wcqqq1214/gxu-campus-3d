...
test('校园边界默认显示，两段闭合轮廓转换为细线且独立于建筑图层', () => {
  const root = createBoundary(data);
  assert.equal(DEFAULT_LAYERS.boundary, true);
  assert.equal(root.children.length, data.rings.length);
  root.children.forEach((line, i) => {
    ...
    assert.ok(
      Math.abs(positions.getX(0) - data.rings[i][0][0]) < 0.001,
    );
    assert.ok(
      Math.abs(positions.getZ(0) + data.rings[i][0][1]) < 0.001,
    );
