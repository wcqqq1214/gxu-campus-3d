...
const cameras = JSON.parse(await fs.readFile(path.join(root, 'docs/model-checks/refinement/s4-paving-cameras.json')));
const camera = cameras.find((c) => c.id === '6b-paving-road-join');
...
async function visit(name, near, light = 'day') {
  ...
  if (near && camera.selected) hash.set('place', camera.selected);
